...
async def choose_option(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect('db.sqlite3') as db:
        async with db.execute('''SELECT day FROM clients WHERE id = ?''', (update.effective_user.id,)) as cursor:
            async for row in cursor:
                day = str(row[0])
    ...
    if update.effective_message.text == "Далее":
        async with aiosqlite.connect('db.sqlite3') as db:
            await db.execute('''UPDATE clients SET day = ? WHERE id = ?''', (1, update.effective_user.id))
            await db.commit()
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Молодец, правильно ответил"
        )
        if day == "1":
                        await context.bot.send_message(
                            chat_id=update.effective_chat.id,
                        ...
                        async with aiosqlite.connect('db.sqlite3') as db:
                            await db.execute('''UPDATE clients SET day = ? WHERE id = ?''', (2, update.effective_user.id,))
                            await db.commit()
                        time_delta = datetime.timedelta(minutes=1)
                        context.job_queue.run_once(send_video2,time_delta,chat_id=update.effective_chat.id, data=update)
        elif day == "2":
                        await context.bot.send_message(
                            chat_id=update.effective_chat.id,
                            text="Следующие видео с заданием придет автоматически через 12 часов"
                        ...
                        async with aiosqlite.connect('db.sqlite3') as db:
                            await db.execute('''UPDATE clients SET day = ? WHERE id = ?''', (3, update.effective_user.id,))
                            await db.commit()
                        time_delta = datetime.timedelta(minutes=1)
                        context.job_queue.run_once(send_video3,time_delta,chat_id=update.effective_chat.id,data=update)
        elif day == "3":
                        await context.bot.send_message(
                            chat_id=update.effective_chat.id,
                        ...
                        async with aiosqlite.connect('db.sqlite3') as db:
                            await db.execute('''UPDATE clients SET day = ? WHERE id = ?''', (4, update.effective_user.id,))
                            await db.commit()
                        time_delta = datetime.timedelta(minutes=1)
                        context.job_queue.run_once(send_task,time_delta,chat_id=update.effective_chat.id, data=update)
        elif day == "4":
                        await context.bot.send_message(
                            chat_id=update.effective_chat.id,
                        ...
                        async with aiosqlite.connect('db.sqlite3') as db:
                            await db.execute('''UPDATE clients SET day = ? WHERE id = ?''', (5, update.effective_user.id,))
                            await db.commit()
                        time_delta = datetime.timedelta(minutes=1)
                        context.job_queue.run_once(send_task,time_delta,chat_id=update.effective_chat.id, data=update)
        elif day == "5":
                        ...
                        async with aiosqlite.connect('db.sqlite3') as db:
                            await db.execute('''UPDATE clients SET day = ? WHERE id = ?''', (6, update.effective_user.id,))
                            await db.commit()
                        time_delta = datetime.timedelta(minutes=1)
                        context.job_queue.run_once(send_task,time_delta,chat_id=update.effective_chat.id, data=update)
        elif day == "6":
                        ...
                        async with aiosqlite.connect('db.sqlite3') as db:
                            await db.execute('''UPDATE clients SET day = ? WHERE id = ?''', (7, update.effective_user.id,))
                            await db.commit()
                        time_delta = datetime.timedelta(minutes=1)
                        context.job_queue.run_once(send_task,time_delta,chat_id=update.effective_chat.id, data=update)
        elif day == "7":
...
async def send_video2( context: ContextTypes.DEFAULT_TYPE):
    ...
    time_delta = datetime.timedelta(seconds=15)
    context.job_queue.run_once(send_task,time_delta,chat_id=update.effective_chat.id,data=job.data)
...
async def send_task(context: ContextTypes.DEFAULT_TYPE):
    ...
    with open('telegram_work_freelance/прогрев.txt','r') as f:
        ...
        async with aiosqlite.connect('db.sqlite3') as db:
            ...
            async with db.execute('''SELECT day FROM clients WHERE id = ?''', (job.data.effective_user.id,)) as cursor:
